...
module.exports = {
  ...
  guard(m) {
    return !m.getFileAndPublishError && !m.getFileAndPublishReady && m.private.stan && m.stanConnected && m.private.hoboClient && m.source && m.healthCheckReady && m.getFileConfig;
  },
  ...
  async execute(m, {
    ...
    const getFile = t => hoboClient.ws(Object.assign({
      headers: {
    ...
    if (token) {
      ...
      try {
        ...
        if (resp && resp.status === 401 && resp.data && resp.data.error === 'invalid_token') {
    ...
    if (response && response.data && response.data.observation_list && response.data.observation_list.length) {
